...
{
	std::string line;
	std::ifstream data ("_data.in");
	std::ofstream data_out;
        data_out.open("_data.out");
	if (data.is_open())
	{
		int i = 0;
		double h_tx__meter, h_rx__meter, f__mhz, P_tx__watt, N_s, d__km, epsilon, sigma;
		int pol;
    		while (getline(data, line))
		{
			if (i == 0)
			{
				h_tx__meter = std::stod(line);
			} else if (i == 1) {
				h_rx__meter = std::stod(line);
			} else if (i == 2) {
				f__mhz = std::stod(line);
			} else if (i == 3) {
				P_tx__watt = std::stod(line);
			} else if (i == 4) {
				N_s = std::stod(line);
			} else if (i == 5) {
				d__km = std::stod(line);
