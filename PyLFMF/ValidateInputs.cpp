    ...
    double N_s, double d__km, double epsilon, double sigma, int pol, double lambda__meter)
{
	...
	if (h_tx__meter > 1.2 * sqrt(sigma) * pow(lambda__meter, 3/2))
		h_tx__meter = 1.2 * sqrt(sigma) * pow(lambda__meter, 3/2);

    if (h_rx__meter < 0)
        return ERROR__RX_TERMINAL_HEIGHT;
		
	if (h_rx__meter > 1.2 * sqrt(sigma) * pow(lambda__meter, 3/2))
		h_rx__meter = 1.2 * sqrt(sigma) * pow(lambda__meter, 3/2);
